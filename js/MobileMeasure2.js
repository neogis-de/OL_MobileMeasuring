MobileMeasure = OpenLayers.Class(OpenLayers.Control, {
    ...
    onClick: function(evt) {
        ...
        switch (button) {
            ...
            case this.finishButton:
				...
				this.map.events.unregister('touchmove', this, this.measure);
				if(navi_control.active==false)
				{
				navi_control.activate()
				}
                this.helpMessageEl.style.display = 'none';
                break;
        }
    ...
    activate: function() {
        ...
        function hide() {
			...
			map.events.unregister('touchmove', null, hide);
			
			
			
        }
        map.events.register('touchmove', null, hide);
    ...
    addFirstPoint: function() {
		...
		if(navi_control.active)
			{
				navi_control.deactivate();
        ...
        this.map.events.register('touchmove', this, this.measure);
		
		this.map.events.register('doubletap', this, this.measure);
    ...
    measure: function(e) {
        ...
        if (!this.line) {
            this.target = new OpenLayers.Feature.Vector(
                new OpenLayers.Geometry.Point(touchpoint.lon, touchpoint.lat)
            );
            ...
            this.target.geometry.x = touchpoint.lon;
            this.target.geometry.y = touchpoint.lat;
            var measure = this.getBestLength(this.line.geometry);
            var labeltext=measure[0].toPrecision(4) +
                ' ' + measure[1];
			var labeltext2=measure[0].toPrecision(4) +
                ' ' + measure[1];
			// DRY
			
				this.target.attributes.label = labeltext;
			document.getElementById("length").innerHTML=labeltext2;
